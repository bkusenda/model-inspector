...
def resize_img(img, w=200, h=None, min_h=None, max_h=None, ignore_resize=False):
    ...
    if img.size[1] < h or img.size[0] < w:
        # if increasing size, use this filter
        ...
        for i in range(val.shape[0]):
            if use_color_for_3channel_data and val.shape[1] == 3:
                ...
                for j in range(val.shape[1]):
                    v = val[i, j]
                    if concat_padding>0 and (val.shape[1] ==1 or val.shape[1]>=3):
                        nv = np.zeros((v.shape[0]+concat_padding,v.shape[1]))
                        nv.fill(-100000)
                        nv[:v.shape[0],:v.shape[1]] = v
                        v = nv
                    xj.append(v)
...
#         xall = to_rgb(normalize(xall, vstats))
    ...
    if val.ndim != 4 and xall.shape[0] * 8 <= xall.shape[1]:
        spt_count = hsplitter(xall.shape[0],xall.shape[1])
        
        if not spt_count is None:
            parts = np.hsplit(xall,spt_count)
            allparts = []
            for i,part in enumerate(parts):
                allparts.append(part)
                if i < spt_count -1:
                    allparts.append(np.zeros((1,part.shape[1],part.shape[2]),dtype=np.uint8))
            xall = np.vstack(allparts)
...
def generate_video(
    ...
    for i, filename in enumerate(image_filenames):
        ...
        size = (width,height)
        if width is not None and width > 0 and  out is None:
#             out = cv2.VideoWriter(video_filename, cv2.VideoWriter_fourcc(*'DIVX'), fps, size)
            out = cv2.VideoWriter(video_filename,cv2.VideoWriter_fourcc(*'avc1'), fps, size)
...
def create_videos_for_session(session_root):
    with open(os.path.join(session_root,"session.json")) as f:
        state_ids = json.load(f)['state_ids']
    image_path = os.path.join(session_root,"0_0","images")
    images_fns = [fn for fn in os.listdir(image_path) if fn.endswith("jpg")]
    os.makedirs(os.path.join(session_root,"videos"),exist_ok=True)
    for imfn in images_fns:
        all_image_paths = [os.path.join(session_root,stateid,"images",imfn) for stateid in state_ids]
        out_file = os.path.join(session_root,"videos",imfn.replace("jpg","mp4"))
        generate_video(all_image_paths,out_file)
    print("Done")